...
class MMModel ( EnergyModel ):
    ...
    def BuildModel ( self, target, log = logFile, ff_type = False  ):
        """Build the model given the parameter set and system information.
        
        Modified by Fernando Bachega (ferbachega@gmail.com) on September 11, 2024
        This is the same method, but it also includes the ff_type argument, 
        important to identify whether DYFF is being used.
        This modification was necessary for the correct assignment of atom types.
        """
        ...
        if self.parameterSetPath is not None:
            ...
            #-----------------------------------------------------------
            #. Modified by Fernando Bachega
            if ff_type == 'DYFF':
                #.Checks if the universal force field is being used
                #.This attribute is generated in MOL2FileReader, modified
                isAtomType_FromMol2 = getattr(target, 'atomType_FromMol2', False)
                
                '''
                Verifies whether the atomic types conform to the Trypos 
                format — as typically exported by Avogadro — and corrects 
                inconsistencies introduced by pDynamo, particularly in 
                the assignment of arginine nitrogen atoms.
                '''
                if isAtomType_FromMol2:
                    #. There is a dictionary that associates trypos 
                    #  atoms with UFF atoms (only some cases).
                    #  check ..pBabel/MOL2AtomTypesToDYFF.py
                    Tripos_type = MOL2AtomTypesToDYFF.TriposMOL2AtomTypes
                    for i, atype in enumerate(target.atomType_FromMol2):
                        if atype in Tripos_type.keys():
...
class MMModelDYFF ( MMModel ):
    ...
    def BuildModel ( self, target, log = logFile ):
        ...
        return super ( MMModelDYFF, self ).BuildModel ( target, log = log, ff_type = 'DYFF' )
