...
#  Module: "Run AutoDock-GPU" window
#
...
#
#  Description:
#      Sibling of prepare_vina_docking.py's "Run AutoDock Vina" window,
#      for AutoDock-GPU instead: SAME receptor/ligand PDBQT preparation
#      and docking-box UI (both reuse util/vina_runner.py's own
#      engine-agnostic helpers unmodified), but AutoDock-GPU needs a
#      PRECOMPUTED AutoGrid4 grid map per receptor before it can dock
#      anything (Vina computes its own search grid on the fly, per
#      job) -- see util/autodock_gpu_runner.py's
#      run_autodock_gpu_docking() for that extra step, and for the
#      GTK-free AutoGrid4/AutoDock-GPU wrapping itself. This file only
#      wires that up to the window's widgets and runs it off the GTK
#      main thread, same division of labour as the Vina window.
#
import gi
...
class PrepareAutoDockGPUWindow:
    ...
    def open_window(self):
        ...
        self.builder.add_from_file(os.path.join(
            self.home, 'src/gui/windows/setup/prepare_autodock_gpu_window.glade'))
        ...
        self.entry_adgpu_command = self.builder.get_object('entry_adgpu_command')
        self.entry_autogrid_command = self.builder.get_object('entry_autogrid_command')
        ...
        self.spinbtn_grid_spacing = self.builder.get_object('spinbtn_grid_spacing')
        self.spinbtn_nrun = self.builder.get_object('spinbtn_nrun')
        self.chk_devnum = self.builder.get_object('chk_devnum')
        self.spinbtn_devnum = self.builder.get_object('spinbtn_devnum')
        self.chk_seed = self.builder.get_object('chk_seed')
        self.spinbtn_seed = self.builder.get_object('spinbtn_seed')
        self.chk_autostop = self.builder.get_object('chk_autostop')
        self.spinbtn_heurmax = self.builder.get_object('spinbtn_heurmax')
        self.spinbtn_nev = self.builder.get_object('spinbtn_nev')
        self.spinbtn_ngen = self.builder.get_object('spinbtn_ngen')
        self.spinbtn_lsit = self.builder.get_object('spinbtn_lsit')
        self.spinbtn_psize = self.builder.get_object('spinbtn_psize')
        ...
        self.on_chk_devnum_toggled(self.chk_devnum)
    ...
    def _sync_executable_paths(self):
        ...
        adgpu_command = gl_parameters.get('adgpu_command')
        if not adgpu_command or not os.path.isfile(adgpu_command):
            # . No reliable PATH name to guess -- see
            #   find_autodock_gpu_executable()'s own docstring -- so
            #   this is very likely to come back empty on a first run;
            #   the user has to Browse for it once, after which it
            #   persists across sessions same as the other executables.
            adgpu_command = adgpu_runner.find_autodock_gpu_executable()
        self.entry_adgpu_command.set_text(adgpu_command or '')

        autogrid_command = gl_parameters.get('autogrid_command')
        if not autogrid_command or not os.path.isfile(autogrid_command):
            autogrid_command = adgpu_runner.find_autogrid_executable()
        self.entry_autogrid_command.set_text(autogrid_command or '')
    ...
    def on_chk_devnum_toggled(self, widget):
        self.spinbtn_devnum.set_sensitive(widget.get_active())
    ...
    def on_button_browse_adgpu_clicked(self, widget):
        self._browse_for_executable(self.entry_adgpu_command)

    def on_button_browse_autogrid_clicked(self, widget):
        self._browse_for_executable(self.entry_autogrid_command)
    ...
    def _get_system_name(self):
        return self.entry_system_name.get_text().strip() or 'adgpu_run'
    ...
    def _resolve_adgpu_command(self):
        adgpu_command = self.entry_adgpu_command.get_text().strip()
        if not adgpu_command or not (os.path.isfile(adgpu_command) and os.access(adgpu_command, os.X_OK)):
            self.main.simple_dialog.info(
                msg='Could not find the AutoDock-GPU executable. Please set it manually (Browse…).')
            return None
        self.vm_session.vm_config.gl_parameters['adgpu_command'] = adgpu_command
        return adgpu_command

    def _resolve_autogrid_command(self):
        autogrid_command = self.entry_autogrid_command.get_text().strip()
        if not autogrid_command or not (os.path.isfile(autogrid_command) and os.access(autogrid_command, os.X_OK)):
            self.main.simple_dialog.info(
                msg='Could not find the AutoGrid4 ("autogrid4") executable. Please set it manually.')
            return None
        self.vm_session.vm_config.gl_parameters['autogrid_command'] = autogrid_command
        return autogrid_command
    ...
    def _get_params(self):
        params = {
            'nrun': int(self.spinbtn_nrun.get_value()),
            'autostop': self.chk_autostop.get_active(),
            'heurmax': int(self.spinbtn_heurmax.get_value()),
            'nev': int(self.spinbtn_nev.get_value()),
            'ngen': int(self.spinbtn_ngen.get_value()),
            'lsit': int(self.spinbtn_lsit.get_value()),
            'psize': int(self.spinbtn_psize.get_value()),
        }
        if self.chk_devnum.get_active():
            params['devnum'] = int(self.spinbtn_devnum.get_value())
    ...
    def on_button_run_clicked(self, widget):
        ...
        if not self._ligand_pdbqt_list:
            ...
            return
        adgpu_command = self._resolve_adgpu_command()
        if adgpu_command is None:
            return
        autogrid_command = self._resolve_autogrid_command()
        if autogrid_command is None:
            return
        work_folder = self._get_work_folder()
        ...
        system_name = self._get_system_name()
        grids_folder = os.path.join(work_folder, system_name + '_GRIDS')
        output_folder = os.path.join(work_folder, system_name + '_OUTPUTS')
        self._output_folder = output_folder
        box = self._get_box()
        spacing = self.spinbtn_grid_spacing.get_value()
        ...
        thread = threading.Thread(
            ...
            args=(adgpu_command, autogrid_command, grids_folder, output_folder,
                  box, spacing, params, self._cancel_event))
        thread.start()
    ...
    def _run_docking_thread(self, adgpu_command, autogrid_command, grids_folder, output_folder,
                             box, spacing, params, cancel_event):
        def progress(event, info):
            GLib.idle_add(self._on_progress, event, dict(info))

        try:
            rows = adgpu_runner.run_autodock_gpu_docking(
                adgpu_command, autogrid_command, self._receptor_pdbqt_list, self._ligand_pdbqt_list,
                grids_folder, output_folder, box, spacing=spacing,
                progress_callback=progress, cancel_event=cancel_event, **params)
        except Exception as error:
            traceback.print_exc()
    ...
    def _on_progress(self, event, info):
        if event == 'grid_start':
            self.label_docking_status.set_text('Generating grid maps for receptor {}/{}: {}...'.format(
                info['index'], info['total'], info['receptor']))
            self._append_log('=== Grid maps for {} ... '.format(info['receptor']))
        elif event == 'grid_done':
            self._append_log('done\n')
        elif event == 'grid_failed':
            ...
            best = min((r['energy'] for r in info['rows']), default=None)
            self._append_log('done (best energy {} kcal/mol)\n'.format(best))
            for row in info['rows']:
                self.liststore_results.append([
                    row['receptor'], row['ligand'], row['run'],
                    row['energy'], row['cluster_rank'], row['cluster_rmsd'],
    ...
    def _on_docking_finished(self, rows):
        ...
        self.label_docking_status.set_text(
            'Finished -- {} result row(s). See the Results tab. Every job\'s command '
    ...
    def on_button_import_previous_results_clicked(self, widget):
        """ Points at a previous "..._OUTPUTS" folder (from this
            tool's own run_autodock_gpu_docking(), which always writes
            an affinity_logs.txt there) and ADDS every row it can
            still find a real docked PDBQT for to the table below,
            alongside whatever is already loaded -- no docking is
            re-run.
        """
        dialog = Gtk.FileChooserDialog(
        ...
        try:
            rows, missing_pairs = adgpu_runner.parse_affinity_log(folder)
        ...
        for row in rows:
            ...
            self.liststore_results.append([
                row['receptor'], row['ligand'], row['run'], row['energy'],
                row['cluster_rank'], row['cluster_rmsd'], -1, -1, -1, -1])
    ...
    def on_button_import_pose_clicked(self, widget):
        """ Imports EVERY pose currently in the Results table, GROUPED
            ...
            its own best energy). Populates every liststore_results
            ...
            See the identical docstring in prepare_vina_docking.py's
            on_button_import_pose_clicked() for the full reasoning: an
            EARLIER version combined every row in the whole table into
            ONE object regardless of ligand name (correct only while
            several differently-named "ligand" rows could exclusively
            mean conformers of one molecule); that broke once ligands
            could be queued/imported independently and a batch could
            genuinely hold several unrelated ligands. Grouping by
            ligand name restores per-ligand-ensemble combining while
            giving every distinct ligand its own object.
        ...
        #   rows (different runs of the same job) share one file.
        models_by_file = {}
        def get_model(row):
            """ None (not an exception) if `row`'s own run has no
                matching MODEL in its docked_pdbqt -- see the same
                helper's docstring in prepare_vina_docking.py for the
                confirmed real-world case this guards against (an
                engine printing a results-table row for a pose it never
                actually wrote to the docked PDBQT).
            """
            ...
            return next((m for m in models if m['model'] == row['run']), None)

        all_skipped = []
        imported_summaries = []

        for ligand_name in group_order:
            group_indices = sorted(groups[ligand_name], key=lambda i: self._results[i]['energy'])
            ...
            if first_model is None:
                all_skipped.append(
                    '{}: best-energy pose (run {}) has no matching MODEL in its PDBQT -- '
                    'nothing imported for this ligand.'.format(ligand_name, best_row['run']))
                continue
            try:
                ...
                vobject = vismol_pdbqt_builder.load_pdbqt_as_vobject(
                    ...
                    tag='adgpu_pose', dedupe_policy='rename')
            except Exception as error:
                traceback.print_exc()
            ...
            for pose_i in group_indices[1:]:
                ...
                model = get_model(row)
                if model is None:
                    all_skipped.append('{} vs {}, run {} (no MODEL for this run in its PDBQT)'.format(
                        row['ligand'], row['receptor'], row['run']))
                    continue
                this_topology = model['atoms']
                if len(this_topology) != len(reference_topology):
                    all_skipped.append(
                        '{} vs {}, run {}: atom count mismatch within the "{}" group ({} vs {}) -- '
                        'stopped combining further poses into this object; {} frame(s) kept.'.format(
                            row['ligand'], row['receptor'], row['run'], ligand_name,
                ...
                if not pdbqt_parser.pdbqt_topology_names_match(reference_topology, this_topology):
                    ...
                    all_skipped.append('{} vs {}, run {} (atom order mismatch)'.format(
                        row['ligand'], row['receptor'], row['run']))
                    continue
            ...
            for frame_index, i in enumerate(imported_indices):
                ...
                vobject.metadata['poses'][frame_index] = {
                    'pose_rank': frame_index, 'score': row['energy'],
                ...
                new_results.append(DockingResult(
                    ...
                    pose_rank=frame_index, score=row['energy'], receptor_id=row['receptor']))
