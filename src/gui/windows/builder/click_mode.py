...
#      position instead of selecting whatever is already there.
#
...
#      cursor (see _read_depth_at_pixel() below) -- the same technique
...
#      _read_depth_at_pixel()'s docstring). If the clicked pixel is
...
def _read_depth_at_pixel ( vm_glcore, mouse_x, mouse_y ):
    ...
    Returns a positive distance-from-camera (float), or None if the
    pixel is background (BACKGROUND_ID / pure white) -- caller should
    fall back to a default depth (e.g. dist_cam_zrp) in that case.
...
def world_pos_from_mouse ( vm_glcore, mouse_x, mouse_y, depth = None ):
    ...
    given explicitly, tries _read_depth_at_pixel() first (the REAL depth
    ...
    if depth is None:
        depth = _read_depth_at_pixel ( vm_glcore, mouse_x, mouse_y )
        if depth is None:
...
def handle_click_to_place_atom ( vm_glcore, mouse_x, mouse_y ):
    ...
    Computes a 3D position from the 2D click (world_pos_from_mouse(),
    proper perspective unprojection -- see its own docstring), converts
    it into the TARGET OBJECT's own local/model space (see below -- this
    step was missing at first, and caused new atoms to land in the wrong
    place as soon as the model had been rotated), and adds one atom
    there via atom_ops.add_atom(). Returns the new Atom, or None if the
    mode isn't actually usable right now (no target object set).
    ...
    wx, wy, wz = world_pos_from_mouse ( vm_glcore, mouse_x, mouse_y )
